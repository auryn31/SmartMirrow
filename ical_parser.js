...
function ical_parser(feed_url, callback){
	...
	this.makeDate = function(ical_date){
		...
		dt.dayname =["Sonntag","Montag","Dienstag","Mittwoch","Donnerstag","Freitag","Samstag"][dt.date.getDay()];
	...
	this.parseICAL = function(data){
		...
		for(var i=0;i<cal_array.length;i++){
			...
			if(in_event){
				...
				if(type.includes('DTSTART') || type.includes('DTEND') || type.includes('DTSTAMP')){
